...
$("#btnSaveCustomer").click(function () {
  ...
  fetch("http://localhost:3457/api/customer-info/create-customer", {
    method: "POST",
...
$("#payButton").click(function () {
  ...
  const nameStaff = $("#staffName").text();
    if (checkFullData() == true) {
      ...
      // Checkout the order
      $.ajax({
        url: "http://localhost:3458/api/order/create-order-checkout-pos",
        type: "POST",
        ...
        success: function (response) {
          ...
          $.ajax({
            url: "http://localhost:3458/api/order/get-order-by-id?orderId=" + orderId,
            method: "GET",
            success: function (data) {
              console.log(data);
              $("#pri_invo a").attr(
                "href",
                `http://localhost:3458/api/order/dow-invoice-pos/${data._id}`
              );
